...
public class Pointage extends BaseEntity {
    ...
    private UserCompagne user;

    @OneToMany(mappedBy = "pointage", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<PointageOperation> listOperation = new ArrayList<>();
     ...
     * 
     * @OneToMany(mappedBy = "pointage", cascade = CascadeType.ALL, orphanRemoval =
     * true)
     * private List<PointageOperation> operations; // Liste des opérations de
     ...
     * System.out.println("Opération : " + operation.getType() + " à " +
     * operation.getHeure());
     * 
     ...
     * erreurPointage = true;
     * } else {
     * totalMinutes += Duration.between(lastEntree.getHeure(),
     * operation.getHeure()).toMinutes();
     ...
     * 
     * // Déclencher une alerte pour demander la correction du pointage si
     * nécessaire
     * if (erreurPointage) {
     * System.out.println("⚠️ Erreur de pointage détectée pour l'utilisateur " +
     * user.getNom() + ". Veuillez corriger les entrées/sorties incohérentes.");
     * }
     * }
     */
